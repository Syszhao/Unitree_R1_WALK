...
UNITREE_MODEL_DIR = "/home/user/Isaaclab/unitree_rl_lab/unitree_model"  # Replace with the actual path to your unitree_model directory
UNITREE_ROS_DIR = "/home/user/Isaaclab/unitree_rl_lab/unitree_ros"  # Replace with the actual path to your unitree_ros package
...
UNITREE_R1_26DOF_CFG = UnitreeArticulationCfg(
    spawn=UnitreeUrdfFileCfg(
        asset_path=f"{UNITREE_ROS_DIR}/robots/r1_description/R1.urdf",
    ),
    init_state=ArticulationCfg.InitialStateCfg(
        pos=(0.0, 0.0, 0.8),
        joint_pos={
            ...
            # === Arms (Asymmetric - Roll) ===
            # 左臂 roll=0.12, 右臂 roll=-0.12
    ...
    actuators={
        # 组1: 强力承重关节 (Hips Pitch/Roll, Knees)
        # 逻辑：Stiffness ~80, Damping ~3-4
        "legs_main_drive": ImplicitActuatorCfg(
            joint_names_expr=[".*_hip_pitch_.*", ".*_hip_roll_.*", ".*_knee_.*"],
            effort_limit_sim=139,  # 需根据R1电机实际参数调整，这里沿用参考值
            velocity_limit_sim=20.0,
            stiffness={
                ".*_hip_pitch_.*": 80.0, # 你的配置: 80
                ".*_hip_roll_.*": 80.0,  # 你的配置: 80
                ".*_knee_.*": 80.0,      # 你的配置: 80
            },
            damping={
                ".*_hip_.*": 3.0,        # 你的配置: 3.0
        ...
        # 组2: 转向与腰部 (Hip Yaw, Waist)
        # 逻辑：Stiffness 35-60, Damping 2.0-3.0
        "legs_yaw_waist": ImplicitActuatorCfg(
            joint_names_expr=[".*_hip_yaw_.*", "waist_.*"],
            effort_limit_sim=88,
            velocity_limit_sim=32.0,
            stiffness={
                ".*_hip_yaw_.*": 60.0,   # 你的配置: 60
                "waist_roll_joint": 40.0, # 你的配置: 40
                "waist_yaw_joint": 35.0,  # 你的配置: 35
            },
            damping={
                ".*_hip_yaw_.*": 2.0,
        ...
        # 组3: 脚踝 (Ankles)
        # 逻辑：Stiffness 20 (放大后), Damping 3.0
        "feet_actuators": ImplicitActuatorCfg(
            joint_names_expr=[".*_ankle_.*"],
            effort_limit_sim=25,
            velocity_limit_sim=37,
            stiffness={
                ".*_ankle_pitch_joint": 20.0, # 你的配置: 20
                ".*_ankle_roll_joint": 20.0,  # 你的配置: 20
            },
            damping=3.0, # 你的配置: 3.0
            armature=0.01,
        ),
        
        # 组4: 上肢与头部 (Arms & Head)
        # 逻辑：保持柔软或较低刚度
        "upper_body_soft": ImplicitActuatorCfg(
            joint_names_expr=[
                ".*_shoulder_.*", 
                ".*_elbow_.*", 
                ".*_wrist_.*", 
